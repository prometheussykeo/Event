...
3. Settings that matter:
   ...
   - **Root Directory:** `./` (leave as-is — the repo root *is* the site root)
   - **Build Command:** leave empty
   - **Output Directory:** leave empty
4. Deploy.

Because the repo root is the site root, `index.html` is always served at `/`. This is
what avoids the `404 NOT_FOUND` you get when a folder is drag-uploaded and the files
end up one level down.
